...
public function viewPackageOrderDetails($id)
{
    ...
    // Find the package order itself
    $package = UserPackage::with(['user', 'package'])
        ->findOrFail($id);

    // Get product orders tied to this package_order_id
    $productOrders = package_product_orders::with('product')
        ->where('package_order_id', $id)
        ->get();

    return view('superadmin.package.vieworderdetails', compact('package', 'productOrders'));
}
...
public function approveTopup($id)
{
   ...
   public function pendingWithdraw()
       ...
       public function payoutpage()
    {
        ...
        return view('superadmin.transaction.payout', compact('transactions'));
    }
...
public function approvePayout($id)
{
   ...
   // $response = Http::withToken(env('PAYSTACK_SECRET_KEY'))
    ...
    if ($response->successful() && isset($response['data']['reference'])) {
        // Save payout as processing with Paystack reference
        $payout->update([
            'status'             => 'processing',
            'paystack_reference' => $response['data']['reference'],
        ]);

        return back()->with('success', 'Payout initiated. Waiting for Paystack confirmation.');
    }

    return back()->with('error', 'Paystack transfer failed: ' . ($response['message'] ?? 'Unknown error'));
}
...
public function bulkApprove(Request $request)
{
    ...
    foreach ($payouts as $payout) {
        ...
        if (empty($user->recipient_code)) {
            continue; // skip users without recipient_code
        }
    ...
    return back()->with('success', 'Bulk payouts initiated. Waiting for Paystack confirmation.');
}
    ...
    if ($event === 'transfer.success') {
        ...
        if ($payout && $payout->status !== 'approved') {
            DB::transaction(function () use ($payout) {
                ...
                // Deduct withdraw_wallet_balance
                $user = $payout->user;
                $user->update(['withdraw_wallet_balance' => 0]);
            });
        }
    }
