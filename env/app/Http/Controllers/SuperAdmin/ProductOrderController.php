...
public function checkout(Request $request)
{
    $request->validate([
        ...
        'member_username'  => 'required_if:order_for,member|exists:users,username',
    ...
    */
    if ($request->payment_method === 'online') {
        ...
        if (!$resp->successful() || ($resp['data']['status'] ?? '') !== 'success') {
        ...
        try {
            ...
            return response()->json([
                ...
                ->with('success', 'Order successful via Paystack.'),
            ]);
...
private function createOrder(
    ...
    foreach ($cart as $item) {
        ...
        // Add CPT to beneficiary
        $beneficiary->increment('total_ctp', $item['cpts'] * $item['quantity']);

        // Distribute to sponsor chain
        $uplineId = $beneficiary->sponsor_id;
        $level = 1;
    ...
    $this->processUnilevelBonus($beneficiary->id, $singletotalAmount);
}
