...
protected function buildBinaryTree($user, $depth = 3)
{
    if (!$user || $depth === 0) return null;

    // Left child
    $leftChild = User::where('upline_username', $user->username)
                     ->where('position', 'left')
                     ->first();

    // Right child
    $rightChild = User::where('upline_username', $user->username)
    ...
    return [
        ...
        'photo' => $user->photo ?? 'default.jpg',
        'position' => $user->position,
        'left' => $this->buildBinaryTree($leftChild, $depth - 1),
        'right' => $this->buildBinaryTree($rightChild, $depth - 1),
    ];
...
protected function buildFullTree23($user, $depth = 3)
{
    if (!$user || $depth === 0) {
        return null;
    }
    ...
    if ($user->leftChild) {
        $left = $this->buildFullTree($user->leftChild, $depth - 1);
        if ($left) {
            $binaryChildren[] = $left;
        }
    }
    ...
    if ($user->rightChild) {
        $right = $this->buildFullTree($user->rightChild, $depth - 1);
        if ($right) {
            $binaryChildren[] = $right;
        }
    }

    // Sponsor children (mark them as sponsor links)
    $sponsorChildren = [];
    foreach ($user->sponsorChildren as $child) {
        if ($child
            ...
            $built = $this->buildFullTree($child, $depth - 1);
            if ($built) {
                $built['is_sponsor'] = true; // mark sponsor nodes
    ...
    return [
        ...
        'photo'      => $user->photo ?? 'default.jpg',
        'position'   => $user->position ?? '',
        'is_sponsor' => $user->is_sponsor ?? false,
        'children'   => array_values(array_merge($binaryChildren, $sponsorChildren)), // clean indexed array
    ];
}
...
protected function buildFullTreenew($user, $depth = 3)
    {
        if (!$user) {
            return null;
        }
        ...
        if ($depth === 0) {
            $hasMore = ($user->leftChild || $user->rightChild || count($user->sponsorChildren) > 0);
...
protected function buildFullTree($user)
{
    if (!$user) {
        return null;
    }
    ...
    // Handle sponsor children (excluding left/right)
    $sponsorChildren = [];
    foreach ($user->sponsorChildren as $child) {
        if ($child
            && $child->id !== optional($user->leftChild)->id
...
public function index()
{
 ...
 $withdrawals = Withdrawal::where('user_id', $user->id)->latest()->get();
   ...
   return view('user.dashboard', compact('genelogy','downlines','orders','user','downlineCount', 'sponsoredCount', 'withdrawaCount', 'bonuses', 'userpack', 'withdrawals', 'walletropup','tree'));
}



private function getAllDescendants($user, $level = 1, &$all = null)
{
    if ($all === null) {
        $all = collect();
    }
    ...
    foreach ($children as $child) {
        ...
        // Recurse deeper
        $this->getAllDescendants($child, $level + 1, $all);
    }
...
private function getAllDownlines($user)
{
    $all = collect();
    ...
    foreach ($directDownlines as $downline) {
        ...
        // Recursively fetch this downline’s own downlines
        $all = $all->merge($this->getAllDownlines($downline));
    }
