...
public function handleMatchingBonus234(User $user, $ctpGained, $generation = 1)
{
    \Log::info("==== [MATCHING START] ==== User: {$user->username} | Gained: {$ctpGained} CPT | Gen: {$generation}");

    if ($generation > 10 || !$user->parent_id) return;
    ...
    // Only match if both legs have at least 16 CPT
    if ($left >= $pairCPT && $right >= $pairCPT) {
        $matchedCPT = min($left, $right);
        $pairs = floor($matchedCPT / $pairCPT);
        ...
        if ($activePackage) {
            ...
            if ($pairs > 0) {
                ...
                //  Deduct matched CPT from both legs
                ...
                \Log::info(" Match limit reached for {$upline->username}");
            }
        } else {
            \Log::info(" No active package found for {$upline->username}");
        }
    } else {
...
public function handleMatchingBonus456(User $user, $ctpGained, $generation = 1)
{
    \Log::info("==== [MATCHING START] ==== User: {$user->username} | Gained: {$ctpGained} CPT | Gen: {$generation}");
    ...
    if ($hasLeftSponsor && $hasRightSponsor) {
        ...
        \Log::info("🚫 No CPT added — {$upline->username} has no valid sponsored leg for {$side}");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1);
    }
    ...
    //  Only match if both legs have CPTs and both legs are sponsored
...
public function handleMatchingBonus5678(User $user, $ctpGained, $generation = 1)
{
    \Log::info("==== [MATCHING START] ==== User: {$user->username} | Gained: {$ctpGained} CPT | Gen: {$generation}");

    //  Stop at 9 generations
    if ($generation > 9 || !$user->parent_id) return;
    ...
    $side = strtolower($user->position);
    if (!in_array($side, ['left', 'right'])) {
        \Log::warning("Invalid side '{$side}' for {$user->username}");
        return;
    }

    // Sponsored legs check
    $hasLeftSponsor  = User::where('sponsor_id', $upline->id)->where('position', 'left')->exists();
    $hasRightSponsor = User::where('sponsor_id', $upline->id)->where('position', 'right')->exists();

    // Add CPT to valid leg
    if ($hasLeftSponsor && $hasRightSponsor) {
        ...
        \Log::info("🚫 No CPT added: {$upline->username} has no valid leg for {$side}");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1);
    }
    ...
    // Count daily matched pairs
    $dailyMatched = \App\Models\Bonus::where('user_id', $upline->id)
        ->where('type', 'matching')
        ->whereDate('created_at', now())
        ->count();

    $remainingPairsForToday = max($matchLimit - $dailyMatched, 0);

    // ==========================
    //   CPT MUST ALWAYS DEDUCT
    // ==========================
    ...
    // If limit reached → DO NOT PAY BONUS
    if ($remainingPairsForToday <= 0) {
        \Log::info("🚫 Daily limit reached: {$upline->username}. CPT deducted but no bonus paid.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1);
    }

    // Pay only within limit
    $payablePairs = min($pairs, $remainingPairsForToday);

    if ($payablePairs > 0) {
        $bonusAmount = 24000 * ($matchPercentage / 100) * $payablePairs;
        ...
        \App\Models\Bonus::create([
            ...
            'type'        => 'matching',
            'is_paid'     => 1,
            'description' => "Matched {$payablePairs} pair(s) (CPT deducted)",
            'is_approved' => false,
        ]);

        \Log::info("💰 Matching Paid: {$upline->username} earned ₦{$bonusAmount} for {$payablePairs} pairs");
    }
...
public function handleMatchingBonus(User $targetUser, $ctpGained, $generation = 1, $side = null)
{
    \Log::info("==> handleMatchingBonus() START: user_id={$targetUser->id}, ctpGained={$ctpGained}, gen={$generation}, side={$side}");

    // Stop recursion after 9 generations
    if ($generation > 9) {
        \Log::info("Stopping recursion: reached generation > 9.");
        return;
    }

    // If no parent/upline, stop
    if (empty($targetUser->parent_id)) {
        \Log::info("Stopping: user {$targetUser->id} has no parent_id.");
        return;
    }

    $upline = User::find($targetUser->parent_id);
    if (!$upline) {
        \Log::warning("Upline not found for parent_id {$targetUser->parent_id}");
        return;
    }

    // Determine side relative to current user
    if ($generation === 1) {
        $side = strtolower($targetUser->position ?? '');
    } else {
        $side = strtolower($targetUser->position ?? $side);
    }

    if (!in_array($side, ['left', 'right'])) {
        \Log::warning("Invalid side '{$side}' for user {$targetUser->id}. Skipping this level but continuing up.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1, $side);
    }

    // Add CPT to the upline's appropriate leg
    if ($side === 'left') {
        $upline->left_ctp_for_matching = ($upline->left_ctp_for_matching ?? 0) + $ctpGained;
        \Log::info("Added {$ctpGained} to LEFT CTP of upline_id={$upline->id}. New left_ctp={$upline->left_ctp_for_matching}");
    } else {
        $upline->right_ctp_for_matching = ($upline->right_ctp_for_matching ?? 0) + $ctpGained;
        \Log::info("Added {$ctpGained} to RIGHT CTP of upline_id={$upline->id}. New right_ctp={$upline->right_ctp_for_matching}");
    }
    ...
    if (!$activePackage) {
        \Log::info("Upline {$upline->id} has no active approved package. Continue upward without paying.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1, $side);
    }

    $package = \App\Models\Package::find($activePackage->package_id);
    if (!$package) {
        \Log::warning("Package not found for userpackage id {$activePackage->id}. Continue upward.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1, $side);
    }
    ...
    if (function_exists('getMatchingSettings')) {
        $settings = getMatchingSettings(strtolower($package->packageName));
        $matchPercentage = $settings['percentage'] ?? 0;
        $matchLimitPairs = $settings['limit'] ?? 0;
    } else {
        $map = [
            'standard'  => ['limit' => 3,  'percentage' => 5],
            'basic'     => ['limit' => 30, 'percentage' => 10],
            'classic'   => ['limit' => 40, 'percentage' => 15],
            'premium'   => ['limit' => 60, 'percentage' => 20],
            'executive' => ['limit' => 70, 'percentage' => 25],
            'vip'       => ['limit' => 80, 'percentage' => 30],
        ];

        $key = strtolower($package->packageName);
        if (isset($map[$key])) {
            $matchLimitPairs = $map[$key]['limit'];
            $matchPercentage = $map[$key]['percentage'];
        }
    }

    $matchLimitPairs = (int) max(0, $matchLimitPairs);
    $matchPercentage = (float) max(0, $matchPercentage);
    ...
    if ($left < $pairCPT || $right < $pairCPT) {
        \Log::info("No complete pair yet for upline {$upline->id}. Left={$left}, Right={$right}. Continue up.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1, $side);
    }

    $possiblePairs = (int) floor(min($left, $right) / $pairCPT);
    if ($possiblePairs <= 0) {
        \Log::info("Calculated possiblePairs=0 for upline {$upline->id}. Continue up.");
        return $this->handleMatchingBonus($upline, $ctpGained, $generation + 1, $side);
    }

    // Deduct CPT only for payable pairs
    ...
    $payablePairs = min($possiblePairs, $remainingPairsAllowedToday);

    $usedCPT = $payablePairs * $pairCPT;
    ...
    \Log::info("Deducted {$usedCPT} CPT (pairs={$payablePairs}) from upline {$upline->id}. New left={$upline->left_ctp_for_matching}, right={$upline->right_ctp_for_matching}");

    if ($payablePairs > 0) {
        $baseAmountPerPair = 24000;
        $bonusAmount = $baseAmountPerPair * ($matchPercentage / 100) * $payablePairs;

        $upline->withdraw_wallet_balance = ($upline->withdraw_wallet_balance ?? 0) + $bonusAmount;
        $upline->save();

        for ($i = 0; $i < $payablePairs; $i++) {
            \App\Models\Bonus::create([
                'user_id'     => $upline->id,
        ...
        \Log::info("Paid matching bonus to upline {$upline->id}: pairsPaid={$payablePairs}, amount={$bonusAmount}");
    } else {
        \Log::info("Daily match limit reached for upline {$upline->id}. No bonus paid today.");
    }
