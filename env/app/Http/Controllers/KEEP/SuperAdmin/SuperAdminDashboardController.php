...
public function adminpostStep(Request $request, $step)
{
    $registration = Session::get('admin_registration', []);
    $validated = [];
            ...
            $request->validate([
                ...
                'sponsor_username' => [
                    function ($attribute, $value, $fail) use ($request) {
                        $uplineUser = User::where('username', $request->input('upline_username'))->first();
                        if (!$uplineUser) {
...
case 5:
    ...
    // 🐞 Debugging: dump finalData before proceeding
    // Comment this out after confirming
    //dd($finalData);

    //  Check required keys exist
    foreach (['first_name', 'last_name', 'email', 'phone', 'state', 'city', 'country', 'address'] as $field) {
        if (!isset($finalData[$field])) {
            return redirect()->route('superadmin.registeruser.step', 1)
                ->withErrors(['error' => "Missing required field: {$field}. Please restart registration."]);
        }
    }
    ...
    //  Create User safely
 $user = User::create([
    'first_name'       => Arr::get($finalData, 'first_name'),
    'last_name'        => Arr::get($finalData, 'last_name'),
    'email'            => Arr::get($finalData, 'email'),
    'phone'            => Arr::get($finalData, 'phone'),
    'state'            => Arr::get($finalData, 'state'),
    'city'             => Arr::get($finalData, 'city'),
    'address'          => Arr::get($finalData, 'address'),
    'country'          => Arr::get($finalData, 'country'),
    'username'         => Arr::get($finalData, 'username'),
    'parent_id'        => $parentId,
    'sponsor_id'       => $sponsorId,
    'position'         => $position,
    'password'         => Hash::make(Arr::get($finalData, 'password')),
    'kin_name'         => Arr::get($finalData, 'kin_name'),
    'kin_phone'        => Arr::get($finalData, 'kin_phone'),
    'kin_email'        => Arr::get($finalData, 'kin_email'),
    'kin_address'      => Arr::get($finalData, 'kin_address'),
    'bank_name'        => Arr::get($finalData, 'bank_name'),
    'account_no'       => Arr::get($finalData, 'account_no'),
    'account_name'     => Arr::get($finalData, 'account_name'),
    'upline_username'  => Arr::get($finalData, 'upline_username'),
    'sponsor_username' => Arr::get($finalData, 'sponsor_username'),
]);
...
if (!empty($data['sponsor'])) {
    ...
    Session::forget('admin_registration');

    return redirect()->route('superadmin.member.allMembers')->with('success', 'User created successfully by admin.');
    }
}
