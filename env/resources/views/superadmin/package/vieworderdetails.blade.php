...
@if($productOrders->count())
        ...
        <p><strong>Amount Paid:</strong> ₦{{ number_format($package->amount_paid, 2) }}</p>
        <p><strong>Payment Method:</strong> {{ $package->payment_method }}</p>
        <p><strong>Approved Date:</strong> {{ \Carbon\Carbon::parse($package->approved_at)->format('F j, Y \a\t h:i A') }}</p>
