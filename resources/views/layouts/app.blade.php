...
document.addEventListener('DOMContentLoaded', function () {
  ...
  uplineInput.addEventListener('input', function () {
    ...
    fetch('/check-upline', {
      ...
      .then(data => {
        ...
        if (data.user.status && data.user.status.toLowerCase().trim() === 'inactive') {
          uplineError.textContent = 'Username found but is inactive. An inactive member cannot be an upline.';
          sponsorInput.required = false;
          return;
        }
        ...
        if (data.leftOccupied && data.rightOccupied) {
          ...
          sponsorError.textContent = 'Sponsor username is required because the upline user has 2 legs fully occupied.';
        } else {
          sponsorInput.required = false;
          sponsorError.textContent = '';
        }
  ...
  sponsorInput.addEventListener('input', function () {
    ...
    fetch('/check-sponsor', {
      ...
      .then(data => {
        if (!data.exists) {
          sponsorError.textContent = 'Sponsor username does not exist.';
          return;
        }

        if (data.user.status && data.user.status.toLowerCase().trim() === 'inactive') {
          sponsorError.textContent = 'Sponsor found but is inactive.';
          return;
        }

        sponsorError.innerHTML = `<span style="color: green;">Sponsor found: ${data.user.first_name} ${data.user.last_name}</span>`;
      })
      .catch(() => {
        sponsorError.textContent = 'Error checking sponsor username.';
      });
  });

  // Trigger check if upline already has a value on load
  if (uplineInput.value.trim() !== '') {
    uplineInput.dispatchEvent(new Event('input'));
  }
});
</script>
