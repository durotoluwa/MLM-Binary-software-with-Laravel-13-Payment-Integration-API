...
<div class="col-sm-4">
  <div class="job-icon pb-4 d-flex justify-content-between">
    <div>
      <div class="d-flex align-items-center mb-1">
        <h2 class="mb-0 lh-1">₦{{ number_format($finalPrice, 2) }}</h2>
      </div>
      <span class="d-block mb-2">
        Package Price: ₦{{ number_format($package->price, 2) }}
      </span>
      @if($finalPrice < $package->price)
        <small class="text-success">
          Upgrade discount applied (₦{{ number_format($package->price - $finalPrice, 2) }} deducted).
        </small>
      @endif
    </div>
  </div>
...
<div class="col-sm-4">
    <div class="job-icon pb-4 d-flex justify-content-between">
        <div>
            <div class="d-flex align-items-center mb-1">
                @php
                    // Find the user's last approved package
                    $lastUserPkg = \App\Models\userpackage::where('user_id', $user->id)
                        ->where('status', 'approved')
                        ->latest('id')
                        ->first();

                    // Default to full bottle count
                    $effectiveBottle = $package->bottle;

                    // If upgrading, deduct previous bottle count
                    if ($lastUserPkg) {
                        $prevBottle = $lastUserPkg->previous_bottle ?? 0;
                        $effectiveBottle = max($package->bottle - $prevBottle, 0);
                    }
                ...
                <h2 class="mb-0 lh-1">
                    ₦{{ number_format($package->apc, 2) }}
                    × {{ $effectiveBottle }} Bottles =
                    ₦{{ number_format($apcTotal, 2) }}
                </h2>
            </div>
            <span class="d-block mb-2">APC Price</span>
        </div>
    </div>
</div>
    ...
    <div class="job-icon pt-4 pb-sm-0 pb-4 d-flex justify-content-between">
        <div>
            <div class="d-flex align-items-center mb-1">
                <h2 class="mb-0 lh-1">
                    ₦{{ number_format($finalPrice + $apcTotal, 2) }}
                </h2>
            </div>
        ...
        <div>
            <div class="d-flex align-items-center mb-1">
                @php
                    ...
                    // If upgrading, deduct previous CPT
                    if ($lastUserPkg) {
                        $prevCpt = $lastUserPkg->previous_package_cpt ?? 0;
                        $effectiveCpt = max($package->cpts - $prevCpt, 0);
                    }
                @endphp

                <h2 class="mb-0 lh-1" id="package-cpt" style="color: green; font-weight: bold;">
                    {{ $effectiveCpt }}
                </h2>
            </div>
...
<div class="card">
<div class="card-body">
<form id="purchaseForm" action="{{ route('user.package.purchase') }}" method="POST" enctype="multipart/form-data">
    @csrf
...
<input type="hidden" name="amount" id="amount" value="{{ $finalPrice + $apcTotal }}">
...
document.getElementById('pay-now-btn').addEventListener('click', function (e) {
    ...
    const handler = PaystackPop.setup({
        ...
        callback: function (response) {
            fetch('{{ route("user.package.purchase") }}', {
                method: 'POST',
                headers: {
