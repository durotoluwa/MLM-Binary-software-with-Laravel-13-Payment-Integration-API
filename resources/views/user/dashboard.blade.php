...
@if($pendingPayments->count() > 0)
    <div class="alert alert-danger d-flex justify-content-between align-items-center">
        <span>
            You have {{ $pendingPayments->count() }} pending registration payment(s).
        </span>
        <a href="{{ route('user.userreg_paymentpage') }}" class="btn btn-sm btn-light">
...
@if(auth()->check() && auth()->user()->is_muted != 1)
    <a class="btn btn-square btn-outline-primary" href="{{ route('user.package') }}">
        Buy Package
    </a>
@endif
                                    </div>
                        ...
                        <div class="widget-stat card">
                            <div class="card-body  p-4">
                                <div class="media ai-icon">
                                    ...
                                    <div class="media-body">
                                        <p class="mb-1">Create Member </p>
@if(auth()->check() && auth()->user()->is_muted != 1)
<a class="btn btn-square btn-outline-primary" href="{{ route('user.downline.step', 1) }}">Create Member</a>
                        ...
                        <div class="widget-stat card">
                            <div class="card-body  p-4">
                                <div class="media ai-icon">
                                    ...
                                    <div class="media-body">
                                        <p class="mb-1">Member Package </p>
                                     
         @if(auth()->check() && auth()->user()->is_muted != 1)
<a class="btn btn-square btn-outline-primary" href="{{ route('user.memberpackage') }}">
    Buy for member</a>
@endif    
...
<div class="widget-stat card">
                            <div class="card-body  p-4">
                                ...
                                <i class="fa-solid fa-wine-bottle"></i>
                                    ...
                                    <div class="media-body">
                                        <p class="mb-1">Member Re-order </p>
                                     @if(auth()->check() && auth()->user()->is_muted != 1)
<a class="btn btn-square btn-outline-primary" href="{{ route('user.order_product') }}">
    Re-order For Member</a> 
@endif  
                            ...
                            <div class="card-body">
                                ...
                                <div class="default-tab">
                                    <ul class="nav nav-tabs" role="tablist">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link" data-bs-toggle="tab" href="#profile"><i class="la la-user me-2"></i> Transactions History</a>
                                        </li>
                                        <li class="nav-item">
...
<table id="responsiveTable" class="display responsive nowrap w-100">
    ...
    <tbody>
@foreach($transactionhistory as $index => $bonus)

    @php
        $paymentStatus = strtolower(trim($bonus->status));

        $isCredit = $paymentStatus === 'credit';
        $isDebit  = $paymentStatus === 'debit';
    ...
    <tr class="{{ $rowClass }}" style="font-weight:700 !important; font-size:12px;">

        <td style="font-weight:700 !important;">
            {{ $index + 1 }}
        </td>

        <td style="font-weight:700 !important;">
            {{ $bonus->user->first_name ?? 'Unknown' }}
        </td>
        ...
        <td>
            <span class="badge {{ $badgeClass }}" 
                  style="font-weight:700 !important; font-size:14px; padding:8px 14px;">
                {{ ucfirst($paymentStatus) }}
            </span>
        </td>

        <td style="font-weight:700 !important;">
            {{ $bonus->created_at->format('d M, Y h:i A') }}
        </td>
...
<tbody>
    @foreach ($genelogy->sortBy('generation_level') as $index => $downline)

    @php
        $isInactive = strtolower($downline->status) === 'inactive';
    @endphp

    <tr 
        @if($isInactive)
            style="color:#ff0000 !important; font-weight:600 !important;"
        @endif
    >

        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ $index + 1 }}
        </td>

        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ $downline->first_name }} {{ $downline->last_name }}
            <br> ({{ $downline->username }})
        </td>
        ...
        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ optional($downline->upline)->first_name ?? '-' }}
            {{ optional($downline->upline)->last_name ?? '' }}
            <br> ({{ optional($downline->upline)->username ?? '' }})
        </td>

        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ optional($downline->sponsor)->first_name ?? '-' }}
            {{ optional($downline->sponsor)->last_name ?? '' }}
            <br> ({{ optional($downline->sponsor)->username ?? '' }})
        </td>

        <td>
            @if($downline->leg === 'Left')
                <span style="background-color:#287af5; color:#fff; padding:4px 8px; border-radius:4px;">
                    Left
                </span>

            @elseif($downline->leg === 'Right')
                <span style="background-color:#054905; color:#ffffff; padding:4px 8px; border-radius:4px;">
                    Right
                </span>

            @else
                <span style="background-color:#6c757d; color:#fff; padding:4px 8px; border-radius:4px;">
                    -
                </span>
            @endif
        </td>
        ...
        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ $downline->current_c_cpts ?? 0 }}
        </td>

        <td style="{{ $isInactive ? 'color:#ff0000 !important; font-weight:600 !important;' : '' }}">
            {{ $downline->current_p_c_cpts ?? 0 }}
        </td>
...
/* ✅ New styles for left vs. right legs */
.left-leg {
    border-color: #2E86C1;      /* Blue border */
