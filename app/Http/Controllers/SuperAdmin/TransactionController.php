...
public function viewPackageOrderDetails($id)
{
    ...
    // Find package order + user + package
    $package = UserPackage::with(['user', 'package'])
        ->findOrFail($id);

    // Get ordered products
    $productOrders = package_product_orders::with('product')
        ->where('package_order_id', $id)
        ->get();

    // Get transaction bonuses/history using transaction_id
    $bonusHistory = collect();

    if (!empty($package->transaction_id)) {
        $bonusHistory = Bonus::with('receiver')
            ->where('transaction_id', $package->transaction_id)
            ->latest()
            ->get();
    }

    return view(
        'superadmin.package.vieworderdetails',
        compact('package', 'productOrders', 'bonusHistory')
    );
}
...
{
    if (!auth()->user()->can('view payout')) {
        abort(403, 'Unauthorized action.');
    }

    // Pending transactions list
    $transactions = Payout::where('status', 'pending')
        ->with('user')
        ->latest()
        ->get();

    //   Totals
    $totalWithdrawals   = Payout::count();
    $totalPaid          = Payout::where('status', 'paid')->count();
    $totalPending       = Payout::where('status', 'pending')->count();
    $totalAmountPaid    = Payout::where('status', 'paid')->sum('amount');
    $totalAmountPending = Payout::where('status', 'pending')->sum('amount');
  $totalDeclined = Payout::where('status', 'declined')->count();
    return view('superadmin.transaction.payout', compact(
        'transactions',
        'totalWithdrawals',
        'totalPaid',
        'totalPending',
    ...
    public function payouthistorypage()
{
    if (!auth()->user()->can('view payout')) {
        abort(403, 'Unauthorized action.');
    }

    // Fetch all payouts except those with 'pending' status
    $transactions = Payout::where('status', '!=', 'pending')
        ->with('user')
        ->latest()
        ->get();

    return view('superadmin.transaction.payout_history', compact('transactions'));
}
...
public function approvePayout($id)
{
    ...
    if (!$user->paystack_recipient_code) {
        return back()->with('error', 'User has no Paystack recipient code.');
    }
    ...
    try {
        ...
        ->post('https://api.paystack.co/transfer', [
            'source' => 'balance',
            'amount' => $payout->amount_payable * 100,
            'recipient' => $user->paystack_recipient_code,
            'reason' => 'User withdrawal payout'
        ]);

        if (!$response->successful()) {
            return back()->with('error', $response['message'] ?? 'Transfer failed');
        }
...
public function bulkApprove(Request $request)
{
    ...
    foreach ($payouts as $payout) {
        ...
        if (!$payout->user->paystack_recipient_code) {
            continue;
        }
        ...
        try {
            ...
            ->post('https://api.paystack.co/transfer', [
                'source' => 'balance',
                'amount' => $payout->amount_payable * 100,
                'recipient' => $payout->user->paystack_recipient_code,
                'reason' => 'Bulk payout withdrawal'
            ]);
    ...
    return back()->with('success','Bulk payout initiated.');
}
...
public function handlePaystackWebhook(Request $request)
{
    ...
    if ($signature !== hash_hmac('sha512', $request->getContent(), config('services.paystack.secret_key'))) {
        abort(403);
    }
