...
$transactionhistory = Bonus::with('user')
    ->where('user_id', $user->id)
    ->whereIn('status', ['credit', 'debit'])
    ->latest()
    ->get();


   // $userpack = userpackage::with(['package', 'bonusesTriggered.receiver'])
   // ->where('user_id', $user->id)
   // ->latest()
   // ->get();

  $userpack = UserPackage::with([
    'package',
    'bonusesTriggered.receiver'
])
->where('user_id', $user->id)
->latest()
->get();
 ...
 $withdrawals = Withdrawal::where('user_id', $user->id)->latest()->get();
  $pendingPayments = User::where('userreg_id', $user->id)
   ...
   return view('user.dashboard', compact('pendingPayments','genelogy','downlines','orders','user','downlineCount', 'sponsoredCount', 'withdrawaCount', 'transactionhistory', 'userpack', 'withdrawals', 'walletropup','tree'));
}
...
public function pacakgepage()
{
    ...
    return view('user.package', compact('all','rank','currentIndex','lastPackagePrice','user'));
}
