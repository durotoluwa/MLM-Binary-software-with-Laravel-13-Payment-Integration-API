...
public function purchase(Request $request)
{
    ...
    // Effective values (deduct previous if upgrading)
   ...
   $validator->after(function ($validator) use ($request, $effectiveBottle, $effectiveCpt) {
    ...
    if ((int)$totalQty !== (int)$effectiveBottle) {
        $validator->errors()->add('product', "You must select exactly {$effectiveBottle} bottles; you selected {$totalQty}.");
    }

    if (round($totalCpts, 2) !== round($effectiveCpt, 2)) {
        $validator->errors()->add('product', "Total CPTs must be exactly {$effectiveCpt}; you selected {$totalCpts}.");
    }
});
...
$userpackage = userpackage::create([
    ...
    'previous_package_cpt' => $newPackage->cpts,   // current package CPT
    'previous_bottle'      => $newPackage->bottle, //  current package bottle count
...
public function handleMatchingBonusold(User $user, $ctpGained, $generation = 1)
{
    if ($generation > 9 || !$user->sponsor_id) {
        return;
    }
...
public function evaluateUserIncentives(User $user): void
{
    ...
    foreach ($ranks as $r) {
        ...
        if ($topSum >= $r->required_ctp) {
            ...
            //  NEW: Check weaker leg percentage condition
